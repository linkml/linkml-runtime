...
class RdfLibDumperTestCase(unittest.TestCase):
    ...
    def test_rdflib_dumper(self):
        ...
        self.assertIn(
            (P["001"], INFO.depicted_by, Literal("https://example.org/pictures/fred.jpg", datatype=XSD.anyURI)), g
        )
        self.assertNotIn(
            (P["001"], INFO.depicted_by, Literal("https://example.org/pictures/fred.jpg", datatype=XSD.string)), g
        )
        ...
        container: Container = rdflib_loader.load(
            OUT, target_class=Container, schemaview=view, prefix_map=self.prefix_map
        )
        self._check_objs(view, container)
    ...
    def test_rdflib_loader(self):
        ...
        view = SchemaView(SCHEMA)
        container: Container = rdflib_loader.load(
            DATA_TTL, target_class=Container, schemaview=view, prefix_map=self.prefix_map
        )
        self._check_objs(view, container)
    ...
    def test_any_of_enum(self):
        ...
        person = rdflib_loader.loads(
            enum_union_type_test_ttl, target_class=Person, schemaview=view, prefix_map=self.prefix_map
        )
        self.assertEqual(person.id, "P:001")
        self.assertEqual(person.age_in_years, 33)
        yaml_dumper.dump(person, to_file=UNMAPPED_ROUNDTRIP)
    ...
    def test_unmapped_type(self):
        ...
        objs = rdflib_loader.from_rdf_graph(graph, target_class=Person, schemaview=view, prefix_map=self.prefix_map)
    ...
    def test_phenopackets(self):
        ...
        for id, expected_uri, prefix_map in cases:
            ...
            self.assertIn(
                Literal(test_label),
                list(g.objects(URIRef(expected_uri))),
